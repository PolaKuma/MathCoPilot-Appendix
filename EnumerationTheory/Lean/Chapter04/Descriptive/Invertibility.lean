...
namespace Chapter04.Invertibility

universe u

/-- The elementary direction of Corollary 4.2.8: an almost-everywhere
measurable inverse supplies, for each measurable set, a measurable preimage
...
theorem inducedSigmaAlgebraSurjective_of_invertibleModNull
    ...
    (hinv : IsInvertibleModNull M) : IsInducedSigmaAlgebraSurjective M := by
  ...
  simp [Chapter00.symmDiff, hparts.1, hparts.2]

/-- The spatial-realization direction of Corollary 4.2.8 on the textbook's
Lebesgue spaces.  A countable measurable point-separating code produces a
full-measure set on which the transformation is injective; Lusin--Souslin then
supplies a measurable inverse, extended harmlessly off its full-measure range. -/
theorem invertibleModNull_of_inducedSigmaAlgebraSurjective
    (M : System.{u}) (hM : Chapter01.IsMeasurePreservingSystem M)
    (hLeb : IsLebesgueProbabilitySpace M.toProbabilitySpace)
    (hsurj : IsInducedSigmaAlgebraSurjective M) : IsInvertibleModNull M := by
  let Md : MeasurableSpaceData.{u} :=
    { X := M.X, measurableSpace := M.measurableSpace }
  letI : StandardBorelSpace M.X := StandardBorel.instanceOfData Md hLeb.2
  ...
  let D : ℕ → Set M.X := fun n =>
    (M.T ⁻¹' B n \ A n) ∪ (A n \ M.T ⁻¹' B n)
  have hD (n : ℕ) : MeasurableSet (D n) :=
    ((hB n).preimage hT).diff (hA n) |>.union <| (hA n).diff ((hB n).preimage hT)
  have hD0 (n : ℕ) : M.μ (D n) = 0 := by
    simpa [D, Chapter00.symmDiff] using hdiff n
  ...
  have hX₀ne : X₀.Nonempty := by
    ...
    have hone : M.μ Set.univ = 1 := hM.1.measure_univ
    rw [hzero] at hone
